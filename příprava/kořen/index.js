...
app.post('/okrskove-mapy', (req, res) => {
    ...
    if(!fs.existsSync(`${__dirname}/volby/${volby}/${prezident}/volebni_okrsky-simple-data-topo.json`)) {
        exec(`cd "volby/${volby}/${prezident}" && echo "" > .server && bash ./volebni_mapy.sh -n && rm .server`, (error, stdout, stderr) => {
            if (error) {
                console.error(`MarkO: program na vytváření volebních map\nVýpis posledního chybového hlášení (${Date()}):\n${error.message}`);
                return res.status(500).send('chyba při spuštění skriptu');
            }
...
app.post('/kandidujici-subjekty', (req, res) => {
    ...
    if (kolo == "1") {prezident = "první kolo"} else if (kolo == "2") {prezident = "druhé kolo"}
    if(druh_map == "vítězné") {
        command = `cd "volby/${volby}/${prezident}" && echo "" > .server && bash ./volebni_mapy.sh -s "základ+koalice" ano && rm .server`
    } else if(druh_map == "samostatné") {
        command = `cd "volby/${volby}/${prezident}" && echo "" > .server && bash ./volebni_mapy.sh -s "všechno" ano && rm .server`
    }
...
app.post('/koalice', (req, res) => {
    ...
    if (druh_voleb == "prezident") {prezident = "první kolo"}
    if (druh_map == "vítězné") {
        command = `cd "volby/${volby}/${prezident}" && echo "" > .server && bash ./volebni_mapy.sh -k "${cisla}" "${nazev_koalice}" "${zkratka_koalice}" && rm .server`
        odpoved = `Vytvoření mapy dokončeno. Pro zobrazení klikněte <a href="volby/${volby}/${prezident}/?koalice" target="_blank">sem</a>.`
    } else if (druh_map == "samostatné") {
        command = `cd "volby/${volby}/${prezident}" && echo "" > .server && bash ./volebni_mapy.sh -koalice-samostatne "${cisla}" "${nazev_koalice}" "${zkratka_koalice}" && rm .server`
...
app.get('/volby/:volby/:kolo/', (req, res) => {
    ...
    if (fs.existsSync(`${__dirname}/volby/${req.params.volby}/první kolo/`)) {
        ...
        res.render(path.join(__dirname, '../public/společné', 'index-samostatne.html'));
    }
});
...
app.get('/volby/:volby/', (req, res) => {
    ...
    if (!fs.existsSync(`${__dirname}/volby/${req.params.volby}/první kolo/`)) {
        res.render(path.join(__dirname, '../public/společné', 'index.html'));
    } else {res.status(404).render(path.join(__dirname, '../public/společné', '404.html'))}
})

app.use((req, res, next) => {
  res.status(404).render(path.join(__dirname, '../public/společné', '404.html'))
})
