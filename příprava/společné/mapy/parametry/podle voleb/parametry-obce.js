...
if(parametry.typzobrazeni == "normální") {
    ...
    parametry.data = parametry.koalice ? `volebni_okrsky-simple-data-topo-${id_obce}-2.json` : `volebni_okrsky-simple-data-topo-${id_obce}.json`
    parametry.strany = parametry.koalice ? `parties-${id_obce}-2.json` : `parties-${id_obce}.json`
    parametry.legendazdroj = parametry.koalice ? `vysledky_cr_${id_obce}_2.json` : `vysledky_cr_${id_obce}.json`
} else if(parametry.typzobrazeni == "účast") {
    parametry.data = `volebni_okrsky-simple-data-topo-${id_obce}.json`
    parametry.urlParams = new URLSearchParams(window.location.search);
