export async function nactenimapy(url) {
  try {
    ...
    if (parametry.typzobrazeni !== "normální" && parametry.urlParams.get('strana') !== null) {
      ...
      document.querySelector("svg").innerHTML = `<text x='50%' y='50%' text-anchor='middle' transform='translate(-50,-50)' fill='red' font-size='16'>Chyba při načítání dat. Adresa ${url} není dostupná. Soubor pravděpodobně neexistuje.</text>`;
    } else if(parametry.typzobrazeni == "samostatné" && parametry.urlParams.get('strana') == null) {
