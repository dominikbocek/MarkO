...
export function info(event, d, strany) {
  ...
  let strana = function (poradi) {return strany.filter(function(element) {if(element["CKAND"] == hlasy[poradi]) {return element["CKAND"]}})[0]}
  ...
  let prvnimisto = {"strana": strana(0) !== undefined ? strana(0)["kandidat"]: "data nejsou dostupná", "podil": strana(0) !== undefined ? `${Math.round((d.properties[hlasy[0].toString()]/d.properties.PL_HL_CELK)*10000)/100} %`: "data nejsou dostupná"}
  let druhemisto = {"strana": strana(1) !== undefined ? strana(1)["kandidat"]: "data nejsou dostupná", "podil": strana(1) !== undefined ? `${Math.round((d.properties[hlasy[1].toString()]/d.properties.PL_HL_CELK)*10000)/100} %`: "data nejsou dostupná"}
  let tretimisto = {"strana": strana(2) !== undefined ? strana(2)["kandidat"]: "data nejsou dostupná", "podil": strana(2) !== undefined ? `${Math.round((d.properties[hlasy[2].toString()]/d.properties.PL_HL_CELK)*10000)/100} %`: "data nejsou dostupná"}
  ...
  if(event.type == "mouseover") {
    tooltip2.html(`<div><b>${udaje["nazev"]}</b><br>`)
  }
}