...
var Svg = d3.select("#mapa")
  ...
  fetch(legenda)
    .then(res => res.json())
    .then(data => {
        for(let i = 0; i < data.length; i++) {window.keys.push(data[i]["kandidat"] + " (" + Math.round(data[i]["proc_hlasu"] * 100) / 100 + "%)"); window.barvy.push(data[i]["color"]);}
  ...
  text.style("alignment-baseline", "middle")
    ...
    //koule.forEach((element) => element.style.fill = barvy);
