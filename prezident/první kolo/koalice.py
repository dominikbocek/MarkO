  ...
  for element in file_data:
      if element["CKAND"] == g:
        break
          ...
          sys.exit(f"Koalice s tímto subjektem již existuje: {puvodnivysledky[g]['JMENO']} {puvodnivysledky[g]['PRIJMENI']}")
...
if not os.path.exists("candidates2.csv"):
    shutil.copyfile("candidates.csv", "candidates2.csv")
strany = pd.read_csv("candidates2.csv", delimiter=",", encoding='utf-8')
kstrana_vstrana = int(strany["CKAND"].iloc[-1] + 1)
a = pd.DataFrame([[kstrana_vstrana, nazevkoalice, zkratka]],
                       columns=['CKAND', 'JMENO', 'PRIJMENI'])
a.to_csv('candidates2.csv', mode='a', index=False, header=False, encoding='utf-8')
...
kstrana_vstrana2 = int(strany["CKAND"].iloc[-1] + 1)
