...
#############################
Help() {
echo "Program na vytváření map s volebními výsledky - verze pro 1. kolo prezidentských voleb"
echo "Nápověda:"
echo
echo "možnosti"
...
exit
   ...
   case $1 in
      ...
      -n) # poběží v normálním režimu, tzn. zpracuje původní výsledky
         ...
         echo "Pro vytvoření samostatných map kandidátů je potřeba nejprve zpracovat data v normálním režimu pomocí příkazu bash ./volebni_mapy.sh -n"
         exit
         fi
	      mkdir "samostatné"
...
cd ..
python3 -m webbrowser "http://127.0.0.1/volby/$(urlencode "$(Url)")/$(urlencode "první kolo")/$(urlencode samostatné)"