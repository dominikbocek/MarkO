...
seznam_kandidatu = "candidates.csv"
seznam_kandidatu_koalice = "candidates2.csv"
seznam_kandidatu_vsechno = "candidates-univerzal.csv"
...
puvodnisoubor = open("candidates.json", 'r', encoding='utf-8')
...
if not os.path.exists(seznam_kandidatu_koalice):
    shutil.copyfile(seznam_kandidatu, seznam_kandidatu_koalice)
strany = pd.read_csv(seznam_kandidatu_koalice, delimiter=",", encoding='utf-8')
kstrana_vstrana = int(POCET_VS + 1)
a = pd.DataFrame([[kstrana_vstrana, nazevkoalice, zkratka]],
                       columns=['CKAND', 'JMENO', 'PRIJMENI'])
a.to_csv(seznam_kandidatu_koalice, mode='a', index=False, header=False, encoding='utf-8')
if not os.path.exists(seznam_kandidatu_vsechno):
    shutil.copyfile(seznam_kandidatu, seznam_kandidatu_vsechno)
strany = pd.read_csv(seznam_kandidatu_vsechno, delimiter=",", encoding='utf-8')
kstrana_vstrana2 = len(strany) + 1
a = pd.DataFrame([[kstrana_vstrana2, nazevkoalice, zkratka]],
                       columns=['CKAND', 'JMENO', 'PRIJMENI'])
a.to_csv(seznam_kandidatu_vsechno, mode='a', index=False, header=False, encoding='utf-8')
