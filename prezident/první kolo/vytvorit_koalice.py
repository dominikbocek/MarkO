...
strany = strany[~strany["CKAND"].isin(koalice)]
strany["CKAND"] = range(1, len(strany) + 1)
strany.to_csv("candidates2.csv", index=False, encoding='utf-8')

strany_vsechno = pd.read_csv(seznam_kandidatu_vsechno, delimiter=",", encoding='utf-8')
kstrana_vstrana2 = len(strany_vsechno) + 1
a = pd.DataFrame([[kstrana_vstrana2, nazevkoalice, zkratka]], columns=['CKAND', 'JMENO', 'PRIJMENI'])
strany_vsechno = pd.concat([strany_vsechno, a])
strany_vsechno.to_csv(seznam_kandidatu_vsechno, index=False, encoding='utf-8')
...
def statistiky(zpracovani):
    ...
    universal[len(strany_vsechno)] = data[koalice].sum(axis=1) # přebírání součtu shora
