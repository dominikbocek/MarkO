#!/bin/bash
# MarkO: program na vytváření volebních map - verze pro sněmovní volby

#############################
# Ověření existence souborů #
#############################
Overeni() {
seznam_souboru=("pscoco.csv" "psrkl.csv" "pst4.csv" "pst4p.csv" "barvy.csv" "vysledky.xml")
...
exit
   ...
   case $1 in
      ...
      -s) # vypíše strany
         ...
         python3 ./vypsat_strany.py --vypsat "$2" --json "$3"
         exit;;
      -k) # uvoří koalice
         Overeni
         if ! test -f "volebni_okrsky-simple-data-topo.json"; then
         ...
         odpoved="$(python3 ./koalice.py --koalice $2 --nazevkoalice "$3" --zkratka "$4")"
      ...
      -koalice-samostatne) # vytvoří libovolné koalice, aniž by se to projevilo na mapách okrskových vítězů
         ...
         odpoved="$(python3 ./koalice_samostatne.py --koalice $2 --nazevkoalice "$3" --zkratka "$4")"
         if [ "$odpoved" == "" ]; then exit; fi
         if [ "$2" == "" ] || [ "$3" == "" ] || [ "$4" == "" ]; then echo "Nezadali jste potřebné parametry."; exit; fi
         python3 ./csvtojson.py --koalice univerzal
         bash ./samostatne.sh -k "$odpoved"
         exit
         ;;
      -i) # informace o programu
         ...
         echo "   a) vytváří mapu volebních vítězů na úrovni okrsků. Každý kandidující subjekt má vlastní barvu. Je to totéž, co ukazují v televizi nebo v článcích na internetu."
...
if [ "$1" == "-n" ]; then
   ...
   if ! test -f "statistics-univerzal.csv"; then
      cp statistics.csv statistics-univerzal.csv
   fi
   ...
   python3 -m webbrowser "http://127.0.0.1/volby/$(urlencode "$(Url)")"
fi
if [ "$1" == "-k" ]; then
   python3 ./odstranit_cleny_koalice.py --koalice "$2"
   ...
   python3 ./csvtojson.py --koalice ano
   python3 ./csvtojson.py --koalice univerzal
   python3 ./pridat_barvy.py --koalice ano
   python3 ./kopirovat_barvy.py --koalice ano
   echo "var nazev = '$(Url)'" > script.js
   python3 -m webbrowser "http://127.0.0.1/volby/$(urlencode "$(Url)")/koalice.html"
fi